...
class BaseSSL(nn.Module):
    ...
    """
    DATA_ROOT = os.environ.get('DATA_ROOT', '/home/liweiquan/data/cifar10')
    IMAGENET_PATH = os.environ.get('IMAGENET_PATH', '/home/liweiquan/data/tiny-imagenet-200')
    ...
    def prepare_data(self):
        ...
        if self.hparams.data == 'cifar':
            ...
            valdir = os.path.join(self.IMAGENET_PATH, 'val-m')
    ...
    def add_model_hparams(cls, parser):
        parser.add_argument('--data', help='Dataset to use', default='imagenet')
        parser.add_argument('--arch', default='ResNet18', help='Encoder architecture')
        parser.add_argument('--batch_size', default=255, type=int, help='The number of unique images in the batch')
...
class SimCLR(BaseSSL):
    ...
    def add_model_hparams(cls, parser):
        # loss params
        parser.add_argument('--temperature', default=1, type=float, help='Temperature in the NTXent loss')
        # data params
...
class SSLEval(BaseSSL):
    ...
    @BaseSSL.add_parent_hparams
    def add_model_hparams(cls, parser):
        parser.add_argument('--test_bs', default=255, type=int)
    ...
    def step(self, batch):
        ...
        acc_5 = []
        zex = p.topk(5).indices
        for i in range(len(y)):
            acc_5.append(y[i] in zex[i])
        acc_5 = torch.Tensor(acc_5)
        acc5 = acc_5.float()
