...
os.environ['CUDA_VISIBLE_DEVICES'] = '0, 1, 2'      #指定需要的GPU
...
def add_learner_params(parser):
    ...
    parser.add_argument('--opt', default='sgd', help='Optimizer to use', choices=['sgd', 'adam', 'lars'])
    ...
    # trainer params
    parser.add_argument('--save_freq', default=3910, type=int, help='Frequency to save the model')
    parser.add_argument('--log_freq', default=391, type=int, help='Logging frequency')
    parser.add_argument('--eval_freq', default=3910, type=int, help='Evaluation frequency')
...
def main_worker(gpu, ngpus, args):
    ...
    # create model
    model = models.REGISTERED_MODELS[args.problem](args, device=device).to(device)
