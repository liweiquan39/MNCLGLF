...
class LinearImageNetSolver(SSLSolver):
    ...
    def pre_train(self):
        ...
        self.num_classes = self.config.model.kwargs.get('num_classes', 100)
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_data['loader']):
            ...
            if (curr_step % self.config.saver.val_freq == 0 or curr_step == total_step) and curr_step > 0:
                ...
                best_prec5 = max(metrics.metric['top5'], best_prec5)
                # testing logger
                ...
                self.logger.info('best acc1:' + str(best_prec1) + '  best acc5:' + str(best_prec5) + '\n')
    ...
    def test(self):
        self.pre_train()
        metrics = self.evaluate()
        self.logger.info('acc1:' + str(metrics.metric['top1']) + ' acc5:' + str(metrics.metric['top5']) + '\n')
