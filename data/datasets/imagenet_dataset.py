...
class ImageNetDataset(BaseDataset):
    ...
    def __init__(self, root_dir, meta_file, transform=None,
                 read_from='ceph', evaluator=None, image_reader_type='pil',
                 server_cfg={}):
        ...
        if len(server_cfg) == 0:
            ...
            for line in lines:
                #words = line.split('\t')
                #filename, label = words[0], (words[1].strip('\n').split(','))[0]
                filename, label = line.rstrip().split(' ')  # cifar10

                # filename, label = line.rstrip().split('\t')  # TinyImageNet
                self.metas.append({'filename': filename, 'label': label})  # tinyimagenet finetune
        else:
