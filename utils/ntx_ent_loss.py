...
from core.utils.memory_bank import MemoryBankModule
from core.utils import dist2
...
class NTXentLoss(MemoryBankModule):
    ...
    def forward(self, out0: torch.Tensor, out1: torch.Tensor):
        ...
        if negatives is not None:
            ...
            if self.gather_distributed and dist2.world_size() > 1:
                # gather hidden representations from other processes
                out0_large = torch.cat(dist2.gather(out0), 0)
                out1_large = torch.cat(dist2.gather(out1), 0)
                diag_mask = dist2.eye_rank(batch_size, device=out0.device)
            else:
                # single process
                out0_large = out0
                out1_large = out1
            ...
            labels = labels + dist2.rank() * batch_size
